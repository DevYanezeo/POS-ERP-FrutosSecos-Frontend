...
export default function RemoveStockDialog({ 
  ...
  const handleRemove = async () => {
    ...
    if (cantidad > stockLoteActual) return alert(`No puedes quitar más del stock disponible del lote (${stockLoteActual} unidades)`)
    setProcessing(true)
    try {
      const idProducto = product?.idProducto || product?.id
              ...
              {selectedLote && (
                <div className="bg-blue-50 border border-blue-300 p-3 rounded">
                  <div className="text-sm text-blue-800">
                    Stock disponible en el lote seleccionado: <span className="font-bold">{stockLoteActual} unidades</span>
                  </div>
                </div>
              )}
              ...
              {selectedLote && (
                <div className="bg-[#FEF3C7] p-3 rounded">
                  <div className="text-sm text-[#92400E]">
                    El stock del lote disminuirá en <span className="font-bold">{Number(stockToRemove || '0')} unidades</span>
                  </div>
                </div>
              )}
            </>
